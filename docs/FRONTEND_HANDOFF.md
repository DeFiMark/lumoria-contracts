...
- **Generator**: `ProjectGenerated(token, taxHandler, creator, name, symbol, buyFee, sellFee, launchMode)`, `BYOLLaunched(token, tokensForLP, bnbForLP)`, `FlatCurveLaunched(token, flatCurve, hardCap)`.
...
- **FeeReceiver**: `FeeReceived(from, amount)` (every inflow — revenue total), `TradeFeeReceived(token, user, fee, tradeAmount, isBuy)` (hook swaps + FlatCurve contributions), `LaunchFeeReceived(token, user, fee)` (every launch, both modes — flat anti-spam fee), `TokenFeeReceived(token, amount)` (generic path only), `FeesWithdrawn(recipient, amount)`, `RecipientUpdated(old, new)`.
